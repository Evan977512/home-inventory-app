...
function ManageCategory() {
    ...
    const handleEdit = async (category, name) => {
        ...
        try {
            const res = await axios.put(`http://localhost:3000/homeserver/api/categories/${category._id}`, editedCategory);
            if (res.status === 200) {
    ...
    const handleDelete = async (category) => {
        try {
            const res = await axios.delete(`http://localhost:3000/homeserver/api/categories/${category._id}`);
            if (res.status === 200) {
                const Toast = Swal.mixin({
                    toast: true,
                    position: 'top-end',
    ...
    const handleSubmit = async (name) => {
        ...
        try {
            const res = await axios.post('http://localhost:3000/homeserver/api/categories', newCategory);
            if (res.status === 200) {
                const Toast = Swal.mixin({
                    toast: true,
                    position: 'top-end',
    ...
    const fetchCategories = async () => {
        const res = await axios.get('http://localhost:3000/homeserver/api/categories/');
        return res.data;
    };
    ...
    return (
        <>
            ...
            <div className="home">
                ...
                {addCategoryClicked === true ? <AddCategory handleSubmit={handleSubmit} /> : null}
                {categories ? (
                    <Categories categories={categories} handleEdit={handleEdit} handleDelete={handleDelete} />
                ) : (
